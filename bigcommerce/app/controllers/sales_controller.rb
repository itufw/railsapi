 class SalesController < ApplicationController
  ...
  before_action :confirm_logged_in
    ...
    def view_orders_by_status
      ...
      orders_filtered =  Order.includes([{:customer => :staff}, {:order_products => :product}]).status_staff_filter(@status_id, staff_id).where('products.id IN (?)', product_ids).references(:products)
      @orders = orders_filtered.order('orders.id DESC')



      # need to reduce these to 1
      num_orders = orders_filtered.group('order_products.product_id').count('order_products.order_id')
      product_qty = orders_filtered.group('order_products.product_id').sum('order_products.qty')
      order_totals = orders_filtered.group('order_products.product_id').sum('orders.total_inc_tax')
