...
class Order < ActiveRecord::Base
  ...
  def import_from_bigcommerce(order)
    ...
    params = {'customer_id': order.customer_id, 'status_id': status_id, 'staff_id': customer.staff_id,\
       ...
       'subtotal': order.subtotal_inc_tax.to_f/1.1 + order.discount_amount.to_f + order.coupon_discount.to_f, 'discount_rate': 0, 'discount_amount': order.discount_amount + order.coupon_discount,\
